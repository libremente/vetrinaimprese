...
public class Utils
{
	...
	public String truncateString(String source, int limit)
	{
		if (source == null)
			return source;

		if (source.length() < limit + 1)
			return source;
	...
	{
		System.out.println(anchor("www.google.com"));
	}

	public static String anchor(String url) throws Exception
	{
	...
	{
		return WEB_URL.matcher(linkUrl).matches();
	}
	...
	public static final Pattern WEB_URL = Pattern.compile("((?:(http|https|Http|Https|rtsp|Rtsp):\\/\\/(?:(?:[a-zA-Z0-9\\$\\-\\_\\.\\+\\!\\*\\'\\(\\)"
			...
			+ DOMAIN_NAME + ")" + "(?:\\:\\d{1,5})?)" // plus option port number
			+ "(\\/(?:(?:[" + GOOD_IRI_CHAR + "\\;\\/\\?\\:\\@\\&\\=\\#\\~" // plus
																			// option
																			// query
																			// params
